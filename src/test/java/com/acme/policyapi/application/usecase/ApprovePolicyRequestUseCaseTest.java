...
class ApprovePolicyRequestUseCaseTest {
    ...
    void testExecuteSuccess() {
        ...
        // Act
        approvePolicyRequestUseCase.execute(testPolicyId);

        // Assert
        verify(policyRequestRepository, times(1)).findById(testPolicyId);
        verify(policyRequestRepository, times(1)).save(policyRequest);
        verify(eventPublisher, times(1)).publishPolicyRequestApproved(policyRequest);
    }
    ...
    void testExecutePolicyRequestNotFound() {
        ...
        verify(eventPublisher, never()).publishPolicyRequestApproved(any());
    }
    ...
    void testExecuteFromValidatedStatus() {
        // Arrange - Policy with VALIDATED status (cannot go directly to APPROVED)
    ...
    void testExecuteFromReceivedStatus() {
        // Arrange - Policy with RECEIVED status (cannot go directly to APPROVED)
    ...
    void testExecuteFromApprovedStatus() {
        // Arrange - Policy already in APPROVED status
    ...
    void testExecuteOrderOfOperations() {
        ...
        inOrder.verify(eventPublisher).publishPolicyRequestApproved(policyRequest);
    }
    ...
    void testExecuteWithDifferentInsuranceCategories() {
        // Test with different insurance categories - only valid transitions
        ...
        for (InsuranceCategory category : categories) {
            // Reset policy to PENDING status for each test to avoid transition errors
            policyRequest.setStatus(PolicyRequestStatus.PENDING);
        ...
        // Note: Each loop iteration resets mocks, so individual verifications within the loop are preferred
    }

    @Test
    void testExecuteWithDifferentSalesChannels() {
        // Test with different sales channels - only valid transitions
    ...
    void testExecuteWithDifferentPaymentMethods() {
        // Test with different payment methods - only valid transitions
    ...
    void testExecuteWithLargeInsuredAmounts() {
        ...
        policyRequest.setInsuredAmount(new BigDecimal("10000000.00"));
        policyRequest.setTotalMonthlyPremiumAmount(new BigDecimal("5000.00"));
    ...
    void testExecuteStatusUpdateReason() {
        ...
        // (The specific reason "Pagamento confirmado e subscrição autorizada" is set in the use case)
        verify(policyRequestRepository, times(1)).save(policyRequest);
        verify(eventPublisher, times(1)).publishPolicyRequestApproved(policyRequest);
    }
